...
async def face_identification(
    ...
    recognition_service.process_detect_queue(
...
async def create_embedding(data: EmbeddingCreate):
    ...
    success = recognition_service.create_face_embedding(
...
async def get_embedding(embedding_id: str):
    ...
    embedding = recognition_service.get_face_embedding(embedding_id)
    if not embedding:
...
async def get_all_embeddings(limit: int = 100, offset: int = 0):
    ...
    embeddings = recognition_service.get_all_face_embeddings(limit, offset)
...
async def update_embedding(embedding_id: str, data: EmbeddingUpdate):
    ...
    success = recognition_service.update_face_embedding(
...
async def delete_embedding(embedding_id: str):
    ...
    success = recognition_service.delete_face_embedding(embedding_id)
    if not success:
        raise HTTPException(status_code=404, detail="Embedding not found")
    return {"success": True}
...
async def search_embeddings(query_embedding: List[float], n_results: Optional[int] = 5):
    ...
    results = recognition_service.search_similar_faces(query_embedding, n_results)
...
async def add_user_images(
    ...
    for file in files:
        try:
            ...
            face_image_base64 = recognition_service.convert_image_to_base64(
                face_image
            )
